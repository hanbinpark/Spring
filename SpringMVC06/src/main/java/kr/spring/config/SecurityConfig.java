...
public class SecurityConfig extends WebSecurityConfigurerAdapter{
	...
	@Bean // 우리가 만들어 놓은 MemberUserDetailsService 메모리로 올려 사용하겠다
		...
		http
			.authorizeRequests()  /*요청에 따른 권한을 처리하겠다*/
				.antMatchers("/")  /* 어떠한 경로로 왔을 때 권한처리를 할 것인지 */
					.permitAll()  /*누구나 접근가능하게끔 전체 권한*/
						.and() 	 /*권한을 또 추가하겠다*/
					.formLogin() /*로그인 보안기능 추가*/
						.loginPage("/loginForm.do") /*Spring Security에서 제공하는 로그인 폼이 아닌 우리가 만든 로그인 폼 사용하겠다*/
						.loginProcessingUrl("/login.do") /*해당 url로 요청이 들어왔을 Spring Security 자체 로그인 기능 수행하겠다*/
					...
					/* 로그인도 안하고 특정 페이지에 접근하려고 할 때 특정 url로 요청하겠다 */
