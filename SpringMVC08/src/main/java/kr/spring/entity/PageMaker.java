...
public class PageMaker { // 페이징처리 클래스
	private Criteria cri; // 현재 몇 페이지인지 정보와 게시글을 몇개씩 볼 것인지 그리고
						  // 현재 페이지에서 몇번글부터 시작해야하는 정보를 가지고 있는 객체가 있어야
						  // 페이징처리가 가능하다
	private int totalCount; // 총 게시글의 수 : 총 게시글의 수를 알아야 몇 페이지가 나오는지 알 수 있다
	...
	public void makePagein() {
		...
		// 질문. 내가 현재 7페이지를 보고있다. 그렇다면 우측끝에 있는 마지막 페이지 번호는? 10
		// 질문. 내가 현재 13페이지를 보고있다. 그렇다면 우측끝에 있는 마지막 페이지 번호는? 20
		...
		if(startPage <= 0) {
			startPage = 1; // 혹시라도 startPage가 0보다 작거나 같다면 1부터 시작
		}
